#ifndef BINARYTREE_H
#define BINARYTREE_H
#include "rbtree/include/head4tree.h"
#include <QChar>
#include <QLine>
#include <QPoint>
#include <QString>
...
struct BinTreeNode {
    ...
    // 构造函数
    BinTreeNode(linktree node, QPoint point = QPoint(0, 0),
    ...
    // 构造函数
    BinTreeNode(QVariant data, QPoint point = QPoint(0, 0),
                BinTreeNode *left = NULL, BinTreeNode *right = NULL) {
        this->data = data;
        this->left = left;
        this->right = right;
...
public:
    ...
    QString getInStr();
    QString getPostStr();
    QVector<BinTreeNode *> getLevelStr();
...
protected:
    ...
    QVector<BinTreeNode *> levelNode; // 层次遍历得到的字符串

    int height(BinTreeNode *Node); // 得到树的高度

    void CreateBinTree(QString &str, BinTreeNode *&Node,
                       int &i); // 通过字符串，前序遍历建立树
    void CreateBinTree(linktree ptreenode, BinTreeNode *&Node);
