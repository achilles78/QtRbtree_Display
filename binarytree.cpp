...
bool BinaryTree::Find(QChar ch, BinTreeNode *Node)
{
    ...
    {
        keyParent.push_front(Node->data.toString());
        return true;
    }
    return false;
}
...
void BinaryTree::preOrder(BinTreeNode *Node)
{
    if(Node!=nullptr)
    {
        preStr.append(Node->data.toString());
        preOrder(Node->left);
        preOrder(Node->right);
    }
}
...
void BinaryTree::inOrder(BinTreeNode *Node)
{
    if(Node!=nullptr)
    {
        inOrder(Node->left);
        qDebug()<<Node->data;
        inStr.append(Node->data.toString());
        inOrder(Node->right);
    }
    qDebug()<<inStr;
}
...
void BinaryTree::postOrder(BinTreeNode *Node)
{
    if(Node!=nullptr)
    {
        ...
        qDebug()<<Node->data;
        postStr.append(Node->data.toString());
    }
}
...
void BinaryTree::CreateBinTree(linktree ptreenode, BinTreeNode *&Node)
{
...
BinaryTree::BinaryTree(linktree treeroot)
{
    Treesize=0;
